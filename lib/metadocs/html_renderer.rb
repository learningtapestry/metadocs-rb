    ...
    element_renderer do
      ...
      def render_key_value_table
        rows = metadata_element.metadata.map do |k, v|
          "<tr><td>#{k}</td><td>#{v.render(renderer_id)}</td></tr>"
        end
      ...
      def render_tuple_table
        thead = metadata_element.header_cells.map { |h| h.render(renderer_id) }.join
        rows = metadata_element.metadata.map do |entry|
          tds = entry.map { |_k_, v| "<td>#{v.render(renderer_id)}</td>" }.join
          "<tr>#{tds}</tr>"
        end.join
